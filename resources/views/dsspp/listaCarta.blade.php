      ...
      <div class="solicitud-card filaCarta"
        ...
        <div class="col-12">
          <div class="d-flex justify-content-end gap-2 flex-wrap btn-actions mt-2">
            @if($tieneCarta)
              <a href="{{ route('dsspp.carta.preview', $a->clave_alumno) }}" target="_blank" class="btn-ver-detalle">
                <i class="bi bi-file-earmark-pdf-fill me-1"></i>
                Ver Carta PDF
              </a>
            @else
              <form action="{{ route('dsspp.carta.generar', $a->clave_alumno) }}" method="POST" style="display:inline">
                @csrf
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function filtrar() {
        ...
        tarjetas.forEach(tarjeta => {
            ...
            const textoTarjeta = normalizar(tarjeta.dataset.busqueda);
            const estadoTarjeta = tarjeta.dataset.estado;
            const carreraTarjeta = normalizar(tarjeta.dataset.carrera);

            const coincideTexto = textoTarjeta.includes(textoBuscar);
            const coincideEstado = estadoSeleccionado === "" || estadoTarjeta === estadoSeleccionado;
            const coincideCarrera = carreraSeleccionada === "" || carreraTarjeta === carreraSeleccionada;
